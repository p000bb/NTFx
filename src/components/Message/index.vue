<template>
  ...
  >
    <div
      v-if="visible"
      :class="[
        'fixed top-4 right-4 z-50 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg shadow-lg',
      ...
      :style="{
        maxWidth: message.length > 100 ? 'min(500px, 90vw)' : 'min(400px, 80vw)',
        minWidth: '300px',
        maxHeight: '80vh',
        overflowY: 'auto'
      }"
      ...
      <div class="p-4">
        <div class="flex items-start">
          <div class="flex-shrink-0">
            <CheckCircle v-if="type === 'success'" class="h-5 w-5 text-green-500 dark:text-green-400" />
            <XCircle v-else-if="type === 'error'" class="h-5 w-5 text-red-500 dark:text-red-400" />
            <AlertTriangle v-else-if="type === 'warning'" class="h-5 w-5 text-yellow-500 dark:text-yellow-400" />
            <Info v-else class="h-5 w-5 text-blue-500 dark:text-cyan-400" />
          </div>
          <div class="ml-3 w-0 flex-1 min-w-0">
            <p
              class="text-sm font-medium text-slate-900 dark:text-white break-words whitespace-pre-wrap leading-relaxed"
              :style="{
                wordBreak: message.length > 50 ? 'break-word' : 'normal',

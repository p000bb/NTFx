<template>
  ...
  <div
    :class="[
      'fixed top-0 right-0 h-full w-[420px] bg-white shadow-lg z-50 transition-transform duration-300',
  ...
  >
    <div class="p-0 h-full flex flex-col">
      ...
      <div class="flex-1 p-6 overflow-y-auto">
        <PinConfig v-if="selectedTab === 'pin'" class="h-full flex flex-col" />
        <ChipConfig v-else class="h-full flex flex-col" />
      </div>
    </div>
  </div>
</template>
...
import ChipConfig from "./chipConfig.vue";
import { useI18n } from "vue-i18n";

const { t } = useI18n();
const visible = ref(false);
const tabs = computed(() => [
  { key: "chip", label: t("tools.chipConfig") },
  { key: "pin", label: t("tools.pinConfig") }
]);
const selectedTab = ref("chip");
